...
class TextSR(base.TextBase):
    ...
    def model_inference(self, images_lr, images_hr, model_list, aster, i, pre_srmodel):
        ...
        if self.args.arch == "tsrn":
            ...
            cascade_images = pre_srmodel(images_lr)
    ...
    def train(self):
        ...
        # else:
        #     model_list = [model]
        pre_srmodel = pcan.PCAN(scale_factor=self.scale_factor, width=cfg.width, height=cfg.height,
                                       STN=self.args.STN, mask=self.mask, srb_nums=self.args.srb, hidden_units=self.args.hd_u)

        pre_srmodel = pre_srmodel.to(self.device)

        pretrained_weights = torch.load('/data/cl/TATT_pcan/pcan.pth', map_location=self.device)

        # 将预训练的参数加载到模型中
        pre_srmodel.load_state_dict(pretrained_weights['state_dict_G']) 
        ...
        if self.args.arch in ["tsrn_tl_wmask", "tsrn_tl"] + ABLATION_SET:
            if self.args.use_label:
                ...
                # optimizer_G = self.optimizer_init(model_list + [pre_srmodel], recognizer=aster_student)
        else:
            ...
            for j, data in (enumerate(train_loader)):
                ...
                if not self.args.go_test:
                    ...
                    if self.args.arch == "tsrn":
                        ...
                        cascade_images = pre_srmodel(images_lr)
                        # print(cascade_images.shape)
                        sr_quality_loss = image_crit(cascade_images, images_hr)

                        sr_quality_loss = sr_quality_loss.mean() * 100
                        # cascade_images = images_lr
                        ...
                        # loss_im = loss_img + loss_recog_distill + sr_quality_loss
                        loss_im = loss_img + loss_recog_distill
    ...
    def eval(self, model_list, val_loader, image_crit, index, aster, aster_info, pre_srmodel, data_name=None):
        ...
        for i, data in (enumerate(val_loader)):
            ...
            ret_dict = self.model_inference(images_lr, images_hr, model_list, aster, i, pre_srmodel)
            ...
            # print(images_sr[0].shape)
            if save_img:
                tensor = images_sr[0]

                for i in range(tensor.size(0)):
                    img_tensor = tensor[i]  # 取出第 i 个样本
                    
                    # 如果需要，将4通道图像转换为3通道或单通道图像（这里假设只取前三个通道）
                    img_tensor = img_tensor[:3, :, :].cpu()  # 只取前三个通道作为RGB图像
                    ...
                    # 保存图像
                    image.save(f'/data/cl/TATT_pcan/test_pic/output_images_sr/TTPSR/image_sr_{i}_{label_strs[i]}.png')
                #######################################################################
                ...
                #     # 保存图像
                #     image.save(f'/data/cl/TATT_pcan/test_pic/output_images_lr/aster_pcan/image_lr_{i}_{label_strs[i]}.png')
                ...
                #     # 保存图像
                #     image.save(f'/data/cl/TATT_pcan/test_pic/output_images_hr/aster_pcan/image_hr_{i}_{label_strs[i]}.png')
            ...
            # aster_dict_lr = aster[0]["data_in_fn"](cascade_images[:, :3, :, :])
