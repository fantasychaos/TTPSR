...
class TextBase(object):
    def __init__(self, config, args, opt_TPG=None):
        ...
        self.device = torch.device(self.config.TRAIN.gpu[0] if torch.cuda.is_available() else "cpu")
        alpha_dict = {
            'digit': string.digits,
    ...
    def generator_init(self, iter=-1, resume_in=None):
        ...
        if self.args.arch != 'bicubic':
            ...
            if cfg.ngpu > 1:
                ...
                if self.args.arch == 'sem_tsrn':
                    for k in image_crit.keys():
                        image_crit[k] = torch.nn.DataParallel(image_crit[k], device_ids=cfg.gpu)
                else:
                    image_crit = torch.nn.DataParallel(image_crit, device_ids=cfg.gpu)
    ...
    def MORAN_init(self):
        ...
        MORAN = torch.nn.DataParallel(MORAN, device_ids=cfg.gpu)
    ...
    def Aster_init(self):
        ...
        aster = torch.nn.DataParallel(aster, device_ids=cfg.gpu)
        aster.eval()
